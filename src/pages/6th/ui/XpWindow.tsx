...
export const XpWindow = ({
  ...
  return (
    <motion.div
      ...
      // 바깥 박스에도 라운드를 줘야 그림자가 둥근 모서리를 따라간다(없으면 모서리 밖으로 사각 그림자가 비침)
      className={`pointer-events-auto select-none overflow-hidden rounded-[9px] ${className}`}
      style={{ boxShadow: '0 16px 40px rgba(0,0,0,.5)' }}>
      {/* 타이틀바 = 드래그 핸들 */}
      <div
        onPointerDown={(e) => draggable && controls.start(e)}
        className={`flex h-8 items-center rounded-t-[9px] pl-[10px] pr-[5px] ${
...
const WinButton = ({ kind, onClick }: WinButtonProps) => {
  ...
  return (
    <button
      ...
      style={{
        background: isClose
          ? 'linear-gradient(180deg,#f8c0b0,#e8836b)'
          : 'linear-gradient(180deg,#c3bcf4,#8b7fd9)',
      }}>
